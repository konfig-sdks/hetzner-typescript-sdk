...
import { readableStreamToString, HetznerError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new HetznerError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new HetznerError(e, e.response?.data)
                }
                if (innerError instanceof HetznerError) {
                    // Got "HetznerError" from the above try block
                    throw innerError;
                }
